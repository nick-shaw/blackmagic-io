#!/usr/bin/env python3
"""HDMI loopback test for HDR static metadata signaling.

Requires an HDMI cable connecting an output to an input on the same DeckLink
...
output's HDR static metadata, captures a frame on the HDMI input, and
verifies the metadata round-trip. The HLG case specifically exercises the
HDMI EDID dynamic-range advertisement fix — without that fix the source
strips HDR Static Metadata for HLG signals.
"""
...
TEST_CASES = [
    ...
    (
        "HLG Rec.2020 (requires EDID HLG advertisement)",
        decklink_io.Gamut.Rec2020,
...
def main():
    print("Blackmagic HDMI HDR Metadata Loopback Test")
    print("=" * 70)
    print("Requires an HDMI cable from output to input.")
    ...
    if not input_device.initialize(INPUT_DEVICE_INDEX, decklink_io.InputConnection.HDMI):
        print("ERROR: Failed to initialize input device on HDMI")
