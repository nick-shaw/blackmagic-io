...
- `examples/advanced/display_tpat.py` — `-p bgra` choice. Renders TPAT files through `BlackmagicOutput.display_static_frame()` with `PixelFormat.BGRA`, exercising the new range-aware BGRA output path. The 16-bit promoted image is downshifted to uint8 (`>> 8`, which recovers the original 8-bit values exactly for 8-bit TPAT sources) before output. The `-r` flag has no effect on the BGRA wire output — the SDK fixes the wire range per transport (narrow Y'CbCr on SDI, full R'G'B' on HDMI); for controlled output range use `-p rgb10` / `-p rgb12`.

### Changed (breaking)
- **`BlackmagicOutput.display_static_frame()` and `display_solid_color()` `output_narrow_range` signature: `bool = True` → `Optional[bool] = None`.** Previously the high-level API forced `output_narrow_range=True` for every pixel format when the caller omitted the argument, silently overriding the canonical per-format defaults documented in the README's PixelFormat section and used by the low-level wrappers. Now `None` resolves to the per-format default: True for YUV8 / YUV10 / RGB10, **False for RGB12**. RGB12 callers who omitted `output_narrow_range` previously got narrow encoding (256-3760); they now get full encoding (0-4095). Pass `output_narrow_range=True` explicitly to preserve the old behaviour. Callers that already passed an explicit `True` or `False` are unaffected. Pre-0.18.0b1 the high-level API had to pick a single default because `bool` cannot represent "unspecified", and `True` was the deliberate choice — at the cost of overriding the canonical RGB12 default documented in the README's PixelFormat section. The new `Optional[bool] = None` signature removes that constraint: omitting the argument now defers to the per-format default, so the high-level API and the README finally agree.
- **`BlackmagicInput.capture_frame_as_uint8()` / `capture_frame_as_uint16()` / `capture_frame_as_rgb()` and their `_with_metadata` variants `input_narrow_range` signature: `bool = True` → `Optional[bool] = None`.** Symmetric with the output-side change above and resolves the same asymmetry on the capture path. `None` resolves to a per-source-format default after format detection (inside `_convert_frame_to_int` / `_convert_frame_to_rgb`, since the source format is detected from the wire rather than declared by the caller at the API boundary): True for YUV8 / YUV10 / RGB10, **False for RGB12**, and False for the BGRA-requested-but-RGB10-delivered HDMI 8-bit R'G'B' path. The BGRA-RGB10 default is symmetric with the library's own BGRA → HDMI output behaviour — the SDK transmits BGRA bytes as full-range R'G'B' on HDMI and delivers 8-bit R'G'B' sources back as LSB-padded RGB10, so capturing one symmetric to producing one means assuming full range (the DVI-D "computer signal" convention 8-bit R'G'B' over HDMI follows in practice). RGB12 sources captured with `input_narrow_range` omitted previously had narrow input assumed (incorrect for the typical full-range 12-bit R'G'B' wire); they now have full input assumed. Pass `input_narrow_range=True` explicitly to preserve the old behaviour. The `_with_metadata` variants now record the **resolved** boolean in the returned dict's `'input_narrow_range'` field, not the user-supplied `None`. New HDMI Notes subsection "Capture-side `input_narrow_range` defaults are per-source-format" documents the full resolution table and the BGRA edge cases.
- **Split the C++ `Gamut` enum into `Matrix` + `Gamut`.** The library originally conflated two distinct concepts under `Gamut`: the Y'CbCr matrix (Rec.601/709/2020 coefficient set, signalled on the wire via VPID for SDI and the AVI InfoFrame for HDMI — needed by every Y'CbCr signal, SDR or HDR) and the HDR static-metadata colorimetry bundle (display primaries + white point, conveyed via the HDR Static Metadata InfoFrame — only meaningful for PQ output on tested hardware). Now: `Matrix` is used for the Y'CbCr matrix (used by the eight YUV↔RGB conversion functions and the captured-frame attribute), and `Gamut` reserved for HDR static-metadata signalling. Both enums carry the same `Rec601/Rec709/Rec2020` values; in Python they become distinct types (`isinstance(x, Gamut)` and `isinstance(x, Matrix)` are no longer interchangeable).
...
### Fixed
- Corrected the documentation around mid-stream HDR / matrix metadata changes. The README and the `set_matrix()` / `set_eotf()` / `set_static_metadata()` docstrings previously claimed the setters "must be called before `setup_output()`" and that the wire-update-requires-`display_frame()` behaviour was HDMI-specific. Neither is accurate: the setters update internal state and take effect on the next `display_frame()` call regardless of `setup_output()` ordering, and the snapshot semantics apply to both SDI and HDMI (the SDK reads metadata atomically with the video buffer at frame-emission time on both transports). New regression test `tests/test_hdr_metadata_loopback.py::test_metadata_snapshot_persists_until_display_frame` covers both halves of the corrected contract. Library behaviour is unchanged; the documentation now accurately describes it.
- `BlackmagicOutput._prepare_frame_data()` now validates that `frame_data`'s height and width match the configured display mode, raising `ValueError` on mismatch. Affects both `display_static_frame()` and `update_frame()`. Previously a mismatched array was passed unvalidated to the per-format C++ converter, which read out of bounds (for arrays smaller than the configured area) or consumed only the configured area (for larger arrays) — no error surfaced at the Python boundary. Most likely to bite callers of `update_frame()` who unintentionally resize the array between updates. The `update_frame()` docstring is also expanded to spell out the persistence contract: pixel format, matrix, and range parameters from the most recent `display_static_frame()` call are reused; the new frame must match the original's height and width and use a dtype valid for the configured pixel format.
- `BlackmagicOutput.display_static_frame()` and `display_solid_color()` now honour `input_narrow_range` on the BGRA pixel-format path — previously the parameter was silently ignored, leaving the input bytes untouched regardless of declared range. Narrow-range input is now expanded to full-range R'G'B' (16-235 → 0-255) via `_adjust_range_uint8` before packing into BGRA, mirroring the capture-side range handling added in 0.17.0b5. Sub-blacks (codes 0-15) and super-whites (codes 236-255) in narrow input are clipped at the boundaries (BGRA's 0-255 range cannot represent them), matching the symmetric capture-side caveat documented in HDMI Notes. `output_narrow_range` is now documented as ignored on the BGRA path (a `UserWarning` is issued if a caller explicitly passes it): the SDK's hardware conversion produces transport-specific output range that the library cannot meaningfully override at the buffer level — narrow Y'CbCr 4:2:2 on SDI and full R'G'B' 4:4:4 on HDMI, regardless of parameter. The previous silent-ignore was a real bug rather than designed behaviour. New HDMI Notes subsection "BGRA output is transport-asymmetric on the wire" documents the rationale and points at YUV10 / RGB10 / RGB12 for controlled output range.
