...
- `tools/`: dropped the unused `CMakeLists.txt` (parallel CMake build path that wasn't being used) and simplified `tools/README.md` to document the single Make-based build. `make` works on macOS, Linux, and Windows (with MinGW).
